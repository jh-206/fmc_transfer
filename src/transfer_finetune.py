...
# Start with pretrained RNN, fine tune to OK data
# No time warp, no fine tune
...
if __name__ == '__main__':
    if len(sys.argv) not in [2, 3]:
        ...
        print("Example: python src/transfer_finetune.py etc/thesis_config.yaml")
        print("Example: python src/transfer_finetune.py etc/thesis_config.yaml 17")
        sys.exit(-1)
    ...
    print(f"~"*50)
    print(f"Running Transfer-Learning Fine-Tune with config file: {confpath}")
    if seed is not None:
        reproducibility.set_seed(seed)
        output_dir = osp.join(conf.output_dir, "fine_tune_reps", f"seed_{seed}")
        ...
        reproducibility.set_seed(seed)
        output_dir = osp.join(conf.output_dir, "fine_tune")
    ...
    print(f"Fine Tuning RNN to FM1 data")
    results_1 = {}
    ...
    print(f"Fine Tuning RNN to FM100 data")
    results_100 = {}
    ...
    print(f"Fine Tuning RNN to FM1000 data")
    results_1000 = {}
    rnn.fit(X_train_samples, y_train_samples, validation_data = (XX_val, yy_val), batch_size=params.batch_size, epochs=params.epochs, verbose_fit = True, plot_history=False)
