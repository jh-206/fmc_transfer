# Executable module to analyze results of no transfer Direct RNN train
# Requires existing output: outputs/notransfer_rnn_reps
...
if __name__ == '__main__':
    ...
    reps_dir = "outputs/notransfer_rnn_reps"
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # Data Count Summaries
    fm10 = [r["FM10"] for r in results]
    n_preds = [r["preds10"].shape[0] for r in fm10];      assert len(np.unique(n_preds)) == 1, f"Preds length don't match"
    n_obs   = [r["preds10_intp"].shape[0] for r in fm10]; assert len(np.unique(n_obs)) == 1, f"Observed length don't match"
    tab10 = pd.DataFrame({
        'Metric': ["Start Time", "End Time", "N. Hours", "N. Obs"],
        'Value' : [conf.f_start, conf.f_end, int(n_preds[0]), int(n_obs[0])]
    })
    ...
    # Write output
    print(f"Writing summary counts to: {osp.join(reps_dir, 'fm10_summary_counts.csv')}")
    tab10.to_csv(osp.join(reps_dir, "fm10_summary_counts.csv"))
    print(f"Writing accuracy metrics to: {osp.join(reps_dir, 'fm10_accuracy_testset.csv')}")
    df.to_csv(osp.join(reps_dir, "fm10_accuracy_testset.csv"))

    print(f"Writing median replication report to: {osp.join(reps_dir, 'fm10_median_rep_report.txt')}")
    with open(osp.join(reps_dir, "fm10_median_rep_report.txt"), "w") as f:
        f.write(f"Median replication index: {median_idx}\n")
        f.write(f"Seed directory: {seed_path.resolve()}\n\n")
        f.write("Metrics:\n")

        for m in metrics:
            f.write(f"{m}: {fm10[median_idx][m]}\n")
