# Executable module to analyze results of transfer learning with frozen recurrent layer
# Requires existing output: outputs/freeze_recurrent_reps
...
if __name__ == '__main__':
    ...
    reps_dir = "outputs/freeze_recurrent_reps"
    if not osp.exists(reps_dir):
        print(f"Can't find required output directory: {reps_dir}")
        sys.exit(-1)

    print(f"Summarizing Transfer w Frozen Recurrent Layer from directory: {reps_dir}")
    conf = Dict(read_yml(osp.join(CONFIG_DIR, "thesis_config.yaml")))
    
    # Get all results files across replications
    files = sorted(Path(reps_dir).glob("seed_*/results_freeze_recurrent.pkl"),key=lambda x: int(x.parent.name.split("_")[-1]))
